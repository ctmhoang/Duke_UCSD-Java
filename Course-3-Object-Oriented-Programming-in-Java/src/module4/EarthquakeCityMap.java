...
//		ellipse(50, 125, 15, 15);
		triangle(50 - 10, 125 + 10,50 , 125 -10,50 +10,125+10 );
		fill(255);
...
//		text("Ocean Quake", 75, 225);
	...
	private void printQuakes()
	{
		...
		int totQuakes = quakeMarkers.size();
		for(Marker cm : countryMarkers)
		{
			...
			{
				if(((EarthquakeMarker)qm).isOnLand() && countryName.equals(qm.getProperty("country")))
				{
					counter++;
				}
			}
		...
		System.out.println("OCEAN QUAKES: " + totQuakes);
		...
		if(country.getClass() == MultiMarker.class) {
			...
			for(Marker marker : ((MultiMarker)country).getMarkers()) {
				...
				if(((AbstractShapeMarker)marker).isInsideByLocation(checkLoc)) {
					//Add Country to PointFeature earthquake
