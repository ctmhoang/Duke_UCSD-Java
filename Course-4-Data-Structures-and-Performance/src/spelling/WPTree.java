...
public class WPTree implements WordPath {
  ...
  public WPTree() {
    this.root = null;
    // YOU DID IT FOR ADD WAH? initialize a NearbyWords object
  ...
  public List<String> findPath(String word1, String word2) {
    ...
    while (!queue.isEmpty()) {
      ...
      WPTreeNode tmp = queue.remove();
      List<String> mutateChild = nw.distanceOne(tmp.getWord(), true);

      mutateChild.parallelStream().filter(v -> !visited.contains(v)).forEach(tmp::addChild);

      queue.addAll(tmp.getChildren());
      visited.addAll(
          tmp.getChildren()
              .parallelStream()
              .map(WPTreeNode::getWord)
              .collect(Collectors.toCollection(HashSet::new)));

      if ((res = tmp.getChildWuvName(word2)) != null) return res.buildPathToRoot();
    }
    return null;
  }
...
class WPTreeNode {
  ...
  public WPTreeNode getChildWuvName(String s) {
    return children.parallelStream().filter(n -> n.getWord().equals(s)).findAny().orElse(null);
  }
}
