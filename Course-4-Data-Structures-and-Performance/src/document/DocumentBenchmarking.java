...
public class DocumentBenchmarking {
  ...
  public static void main(String[] args) {
    ...
    for (int numToCheck = start;
      ...
      strTimer = System.nanoTime();
      IntStream.range(0, trials).sequential().mapToObj(i -> new BasicDocument(text)).forEachOrdered(Document::getFleschScore);
      endTimer = System.nanoTime();

      basicDuration = (endTimer - strTimer)/1_000_000_000D;

      strTimer = System.nanoTime();
      IntStream.range(0, trials).sequential().mapToObj(i -> new EfficientDocument(text)).forEachOrdered(Document::getFleschScore);
      endTimer = System.nanoTime();
